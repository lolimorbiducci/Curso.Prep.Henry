...
function deObjetoAmatriz(objeto){
  ...
  for (const propiedad in objeto) {
    objetoEnMatriz.push([propiedad, objeto[propiedad]]);
  };
  return objetoEnMatriz;
}


function numberOfCharacters(string) {
  //La función recibe un string. Recorre el srting y devuelve el caracter con 
...
function capicua(numero){
  ...
  const numInvertido = numero.split("").reverse().join("");
  ...
  for (var i = 0; i<arreglo1.length; i++) {
    for (var j = 0; j<arreglo2.length; j++){
      if (arreglo1[i] === arreglo2[j]) {
        arregloCruzado.push(arreglo1[i]);
      }
    }
  } return arregloCruzado;
}
